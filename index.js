        ...
        applinks: {
            details: [{
                ...
                components: [
                    ...
                    { '/': '/join-captain', comment: 'انتساب الكابتن' }
                ]
            }]
        }
    });
...
// 🛵 /join-captain — رابطٌ واحد يُوزَّع على من نريدهم كباتن.
//
// • التطبيق مثبّت ⇒ App Links (أندرويد) و Universal Links (iOS) تختطفان
//   الرابط قبل المتصفّح، ويتولّاه معالج appUrlOpen فيفتح صفحة الانتساب
//   داخل التطبيق مباشرة.
// • غير مثبّت ⇒ يفتح المتصفّح على الصفحة نفسها، وهي تعمل على الويب
//   بكامل حقولها. وبعد الإرسال تُعرض له بطاقة «حمّل التطبيق» برابط متجره.
//
// ولماذا مسارٌ قصير لا captain-signup.html مباشرةً: الرابط يُكتب على
// ملصقاتٍ ويُقال في الهاتف، و«.html» فيه ما يُخطئ فيه الناقل. والأهمّ أن
// مسارات Universal Links تُعلَن في ملفٍ ثابت، فبقاؤه قصيراً يُبقيها بسيطة.
app.get('/join-captain', (req, res) => {
    res.sendFile(path.join(__dirname, 'public_html', 'captain-signup.html'));
});

...
## 9. نظام الباقات (2026-07-10 — commit d84d009)

نوعان من المتاجر عبر `Place.tier`:

| | أساسي `basic` (افتراضي) | متجر كبير `pro` |
|---|---|---|
| المنتجات والطلبات والمحادثات | نعم | نعم |
| المستحقات وكشف الحساب والتسويات | نعم (أموال التاجر لا تُحجب أبداً) | نعم |
| نقطة البيع | لا | نعم |
| التقارير والتحليلات والأرباح | لا | نعم |
| المخزون المتقدم (حركات/توريد/جرد) | لا | نعم |
| المصروفات | لا | نعم |

- الحماية في السيرفر: `requirePro` يرد 403 برمز `PRO_TIER_REQUIRED` على 16 مساراً.
- الأدمن يبدّل الباقة من جدول المحلات (زر التاج) أو `PUT /api/merchant-erp/admin/places/:id/tier`
  (صلاحية `manage_stores`) — والتاجر يصله إشعار بالتغيير.
- واجهة التاجر: الأساسي يرى بطاقة المحاسبة فقط مع بانر ترقية، وشاشة قفل عند فتح صفحات pro.

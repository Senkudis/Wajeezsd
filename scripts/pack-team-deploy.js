...
3. لا يلزم أي إعداد نطاق. موقع الفريق يُخدَم على wajeezsd.com/team
   بشهادة النطاق الرئيسي نفسها.

⚠️ لا ترفع هذا الأرشيف في مجلد نطاق فرعي. موقع الفريق ليس موقعاً مستقلاً —
   يخدمه تطبيق وجيز نفسه ويقرأ من نفس قاعدة البيانات. مكانه جذر التطبيق فقط.

لماذا لا نطاق فرعي؟
-------------------
تسجيل تطبيق Node في CloudLinux مرتبط بنطاق واحد، ومشاركة جذر المستندات لا
تنقل هذا الارتباط: النطاق الفرعي يُخدَم ملفات ثابتة بلا خادم، فتردّ مسارات
/api بـ 404. وربط تطبيق ثانٍ به يعني عمليةً ثانية تشغّل scheduler.js مرة
أخرى — مهام cron كل دقيقة (توزيع الطلبات، التسويات، الإشعارات) تُنفَّذ مرتين.

لتحويله إلى نطاق فرعي مستقبلاً (يحتاج استضافة تسمح بربط نطاقين بتطبيق واحد):
اضبط TEAM_HOSTS و TEAM_BASE_URL في متغيّرات البيئة — قبل طباعة أي بطاقة.

...
const PromoCodeSchema = new mongoose.Schema({
    ...
    // ── 🏪 متجر التاجر المالك للكود (null = كود إدارة) ──
    //
    // لماذا حقلٌ مستقلّ و places لا تكفي: الإدارة قد تُنشئ كوداً محصوراً
    // بمتجر التاجر (حملة ترويجية تدفعها المنصّة). لو كانت الملكية تُستنتج من
    // places لاستطاع التاجر تعديل ذلك الكود أو حذفه — وهو ليس كوده ولا
    // يتحمّل كلفته. الملكية تُصرَّح ولا تُستنتج.
    //
    // ⚠️ كل كود يحمل هذا الحقل مفروضٌ عليه خادمياً appliesTo: 'products'
    // و places: [هذا المتجر]. السبب محاسبي لا تجميلي: إيراد التاجر يُخصم
    // منه الكوبون فقط حين appliesTo === 'products' (انظر
    // routes/merchant-erp.js). كودٌ على التوصيل أو الإجمالي ينفق من عمولة
    // المنصّة وأجرة الكابتن — أي أن التاجر يوزّع مال غيره.
    merchantPlace: {
        type: mongoose.Schema.Types.ObjectId,
        ref: 'Place',
        default: null,
